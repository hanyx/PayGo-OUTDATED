...
class PayPalIpnCheck {
    ...
    private $subscriptionFormString;
    ...
    public function generateForm($parameters)
    {
        ...
        $this->formString =  '<form name="_xclick" action="' . $this->getEndpoint() . '" method="POST" onSubmit="check()" id="paypal">';
        $this->formString .= '<input type="hidden" name="cmd" id="paycmd" value="_xclick">';
    ...
    public function generateSubscriptionForm($parameters)
    {
        foreach($parameters as $key => $val)
        {
            $this->parameters[$key] = $val;
        ...
        $this->subscriptionFormString =  '<form name="_xclick2" action="' . $this->getEndpoint() . '" method="POST" onSubmit="check()" id="paypalSub">';
        $this->subscriptionFormString .= '<input type="hidden" name="cmd" id="paycmd" value="_xclick-subscriptions">';
        ...
        foreach($this->parameters as $key => $val)
        {
            ...
            $this->subscriptionFormString .= $buildString;

        }

        // Append the submit button and form closing
        $submitValue = $this->parameters['submit'];
        $this->subscriptionFormString .= "</form>";
        return $this;
    }
    ...
    public function printSubscriptionForm()
    {
        echo $this->subscriptionFormString;
    }
