...
if (count($url) == 5) {
	if ($url[3] == "view") {
		...
		if ($currentMessage->read($url[4]) && $currentMessage->getFolder() == MessageFolder::INBOX && $currentMessage->isOwner($uas->getUser()->getId())) {
            ...
            $response['sender'] = $currentMessage->getSender();
            $response['id'] = $currentMessage->getId();
    ...
    <table class="table table-hover table-vcenter pi-table bigger-but-not-huge-table table-messages">
        ...
        foreach ($messages as $message) {
            ...
            <tr data-id="<?php echo $message->getId(); ?>" class="message-modal-launcher">
                <td class="hidden-xs" style="width: 140px;"><?php echo $message->getSender(); ?></td>
    ...
    <script>
        $(function() {
            $('.message-modal-launcher').click(function() {
                $.post('/seller/messages/inbox/view/' + $(this).attr('data-id'), {}, function(data) {
                    ...
                    $('#reply').attr('href', '/seller/messages/reply/' + data.id);
                });
            });
        });
    </script>
