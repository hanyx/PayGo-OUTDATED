			...
			<aside class='sidebar sidebar-large'>
				<div class='list-group list-normal m-t-n-xmini scroll-y scrollbar' style='max-height:400px'>
					...
					foreach ($messages as $message) {
						echo '<a href=\'/seller/messages/sent/view/' . $message->getId() . '\' class=\'list-group-item ' . ($viewMessage ? ($currentMessage->getId() == $message->getId() ? 'active' : '') : '') .  '\'><small class=\'pull-right text-muted\'>' . formatTime(strtotime($message->getDate())) . ' ago</small>' . $message->getRecipient() .  '<br><small>' . $message->getExcerpt() . '</small></a>';
					}
					?>
				</div>
			</aside>
